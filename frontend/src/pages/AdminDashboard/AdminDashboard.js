...
//                     <a href="/create-blog" className="action-button">
...
//                               <a href={`/blog/${post._id}`} className="table-action-button view">View</a>
//                               <a href={`/edit/${post._id}`} className="table-action-button edit">Edit</a>
//                             </td>
//                           </tr>
//                         );
//                       })}
...
import { useUser } from "../../context/UserContext"
import PermissionsList from "../../components/blogComponents/PermissionsList/PermissionsList"
import SectionManager from "../../components/sections/SectionManager/SectionManager"
import "./AdminDashboard.css"
      ...
      <div className="dashboard-grid">
        <div className="dashboard-card">
          ...
          <div className="dashboard-card-body">
            <div className="stats-grid">
              <div className="stat-item">
                <div className="stat-value">{blogs.length ?? 0}</div>
                <div className="stat-label">Total Posts</div>
              </div>
              <div className="stat-item">
                <div className="stat-value">{mockUsers?.filter((user) => user.role === "contributor").length ?? 0}</div>
                <div className="stat-label">Contributors</div>
              </div>
              <div className="stat-item">
                <div className="stat-value">{permissionRequests?.filter((req) => req.status === "pending").length ?? 0}</div>
                <div className="stat-label">Pending Requests</div>
              </div>
            </div>
                ...
                {blogs.slice(0, 5).map((post) => {
                  ...
                  return (
                    <tr key={post._id}>
                      ...
                      <td>{postDate.toLocaleDateString()}</td>
                      <td className="post-actions">
                        <a href={`/blog/${post._id}`} className="table-action-button view">
                          View
                        </a>
                        <a href={`/edit/${post._id}`} className="table-action-button edit">
                          Edit
                        </a>
                      </td>
                    </tr>
        ...
        <section className="admin-dashboard">
          <div className="container">
            <div className="admin-header">
              <h1>Admin Dashboard</h1>
              <p>Manage your website content and blog posts</p>
            </div>
            ...
            <div className="admin-content">
              {activeTab === "dashboard" ? renderDashboardContent() : <SectionManager activeSection={activeTab} />}
            </div>
          </div>
        </section>
      </main>
